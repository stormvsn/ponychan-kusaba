...
function domReady() {
	...
	try {
		...
		var timezone = timenow.getUTCOffset();
		var tzHours = parseInt(timezone.substring(0,3));
		var tzMinutes = parseInt(timezone.substring(0,1) + timezone.substring(3,5));
		var serverDST = true; //find a way to detect this based on timenow?
		var timeFormat = 'ddd, MMM d, yyyy ' + (getCookie('twelvehour') !== '0' ? 'h:mm tt' : 'H:mm');
		$$('.posttime').each(function (span) {
			span.innerHTML = Date.parse(span.innerHTML).addHours((serverDST ? 7 : 8) + tzHours).addMinutes(tzMinutes)
	...
	$$('#delform [name=reportpost]')[0].onclick = function (e) {
