...
function set_global_stylesheet(name) {
	//var link = document.getElementById("globalstylelink");
	var link = null;
	var links = document.getElementsByTagName("link");
	for(var i = 0; i<links.length;i++) {
		if(links[i].getAttribute("href") == "http://www.ponychan.net/chan/css/img_global.css") {
			link = links[i];
			break;
		}
	}
	if(link) {
		link.setAttribute("href", "http://www.ponychan.net/chan/css/img_global_" + name + ".css");
	}
}
...
if(getCookie('altglobalstyle')) {
	set_global_stylesheet(getCookie('altglobalstyle'));
}
...
function showThemeMenu() {
	try {
		...
		$('setting-tz').value = getCookie('timezone') || '-8';
		...
		$('setting-sidebarnav').checked = getCookie('altglobalstyle') === 'sidebar';
...
function saveSettings() {
	try {
		...
		var tz = $('setting-tz').value;
		var tzn = parseInt(tz);
		if (tz !== tzn.toString() || Math.abs(tzn) > 12)
			return alert('The timezone must be a number from -12 to +12.');
		set_cookie('timezone', tz, 365);
		...
		if ($('setting-sidebarnav').checked) {
			set_cookie('altglobalstyle', 'sidebar', 365);
		} else {
			del_cookie('altglobalstyle');
		}
...
function domReady() {
	...
	try {
		...
		$$('.posttime').each(function (span) {
			span.innerHTML = Date.parse(span.innerHTML).addHours(8 + timezone).toString(timeFormat)
